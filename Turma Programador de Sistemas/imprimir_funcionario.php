<?php
	...
	//cria um documento PDF - configrações do pape
	...
	//espaço linhas, conteudo, contorno 1,0, quebra linha 1,0 , direção RCL	
	$fpdf->Cell(0,05,"Senac PR",1,1,'C');
	$fpdf->Cell(0,05,converte("Endereço Senac"),0,1,'L');
	...
	$fpdf->Cell(0,5,converte("Relatorio de Todos os funcionários"),0,1,'C');
	...
	while($linha = mysqli_fetch_assoc($dados)){
    	...
    	//configura a fonte
		$fpdf->setFont('arial','B',12);
		$fpdf->Cell(0,05, converte($linha['nome']) ,0,1,'L');
    	...
    	//configura a fonte
		$fpdf->setFont('arial','B',12);
		$fpdf->Cell(0,05,converte($linha['salario']),0,1,'L');
	...
	//Query: mostrar todos os FUNCIONARIOS
	$sql = "SELECT * FROM funcionario order by salario";
	$dados = mysqli_query($con, $sql);
